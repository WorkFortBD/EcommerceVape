...
import CategoryWiseProductList from "./CategoryWiseProductList";
...
const CategoryWiseProductContainer = ({
  isMainCategory,
  subCategories,
  mainCategoryBanner,
}) => {
  ...
  useEffect(() => {
    ...
    if (router.query?.["category"]) {
      dispatch(
        getCategoryOrBrandDetails("categories/" + router.query?.["category"])
      );
    }
    ...
    <>
      ...
      <section className="pt-2 pt-md-4">
        {(categoryBrandDetails.banner_url ||
          ...
          typeQuery === "haat-bazaar" ||
          typeQuery === "super-sale" ||
          isMainCategory) && (
        ...
        <div className={`row ${isMainCategory ? "my-4" : ""}`}>
          {isMainCategory &&
            subCategories?.map?.((item, index) => {
              return (
                <div
                  className="col-lg-3 col-md-4 col-6 py-2 py-md-3 px-3 px-md-3 pb-3 m-0 mb-md-3"
                  key={index}
                >
          ...
          {!isMainCategory && (
            <div className="col-md-12 mb-5 px-0" style={{ fontSize: "14px" }}>
              <CategoryWiseProductList
                ...
                showFilterHandler={() => setShowFilter((preState) => !preState)}
              />
...
export default CategoryWiseProductContainer;
