              ...
              <div className="d-flex justify-content-between">
                <div className="details_heading">
                  ...
                  <small className="text-secondary">
                    Placed on{" "}
                    {dayjs(orderDetails.transaction_date).format(
                      "dddd, MMMM Do YYYY"
                    )}
                  </small>
                </div>
                <div className="">
                  <small className="order_total_price">
                    <span className="text-secondary">Due Total</span>&nbsp;
                    &nbsp; : {formatCurrency(orderDetails.due_total)}
                  </small>
                  <small className="order_total_price">
                    <span className="text-secondary">Paid Total</span>&nbsp;
                    &nbsp; : {formatCurrency(orderDetails.paid_total)}
                  </small>
                  <small className="order_total_price">
                    <span className="text-secondary">Grand Total </span> :{" "}
                    {formatCurrency(orderDetails.final_total)}
                  </small>
                </div>
              </div>
            </div>
            {isLoading && <TableShimmer count={4} />}
