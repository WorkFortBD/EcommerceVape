...
function priceCalculation(product) {
  ...
  const is_offer_enable = productHasOffer(
    sellingPrice,
    offerPrice,
    isOfferEnable
  );
  const selling_price =
    typeof sellingPrice !== "undefined" && sellingPrice !== null
      ? sellingPrice
      : 0;
  const default_price =
    is_offer_enable && offerPrice != 0 && offerPrice !== null
      ? offerPrice
      : selling_price;
  const offer_price =
    is_offer_enable && offerPrice != 0 && offerPrice !== null ? offerPrice : 0;
