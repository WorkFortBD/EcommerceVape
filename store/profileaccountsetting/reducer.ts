  ...
  switch (action.type) {
    ...
    case Types.GET_SINGLE_ADDRESS:
      ...
      if (action.payload.type === "billing_address") {
        cloneAddress = state.billingAddress.filter(
          (item) => item.id === action.payload.id
        );
      } else {
        cloneAddress = state.shippingAddress.filter(
          (item) => item.id === action.payload.id
        );
      }
      ...
      if (cloneAddress.length > 0) {
        cloneSelectedAddress = {
          ...
          is_default_selected:
            +cloneAddress[0].is_default === 1
              ? { label: "Yes", value: +cloneAddress[0].is_default }
              : { label: "No", value: +cloneAddress[0].is_default },
          selectedCountry: {
            label: cloneAddress[0].country,
            value: +cloneAddress[0].country_id,
          },
          selectedDivision: {
            label: cloneAddress[0].division,
            value: +cloneAddress[0].division_id,
          },
          selectedCity: {
            label: cloneAddress[0].city,
            value: +cloneAddress[0].city_id,
          },
          selectedArea: {
            label: cloneAddress[0].area,
            value: +cloneAddress[0].area_id,
          },
